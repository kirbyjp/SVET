...
### $\Delta H$ — "Coherence-Shedding"
**Type:** Derived Quantity (Bookkeeping Outcome)  
**Definition:** The portion of coherent update demand that a node cannot instantiate and therefore rejects and diverts out of the coherent ledger when local exactions exceed the available processing budget [1].  
...
**Definition:** The local, deterministic division of incoming exaction demand resolved by the update rule [1].  
**Forensic Role:** Clarifies that $\Delta H$ (coherence-shedding residue) has no independent physical agency; it is simply the unresolvable remainder when local exactions exceed the available budget [1]. Instantiation is the passive, deterministic resolution performed by the update rule when the coherent portion of the exaction load can be satisfied [1]. Total exaction is strictly conserved under the single-equation ledger identity:  
$$I_{\text{in}} = I_{\text{refl}} + I_{\text{trans}} + \Delta H = 100\%$$  
This ensures that even during structural failure or decay, no update demand is ever lost or created [1].
...
| **Constraint Conflict** | Binding Energy | The observable projection of reduced routing strain in a shared routing configuration [1]. | Layer 1/2: Constraint-Driven Ledger Event |
| **Constraint Resolution** | Scattering Cross-Sections / Collisions | Local budget and strain-bias reallocations that occur when the update rule resolves a constraint conflict, producing measurable scattering angles and probabilities [1]. | Layer 1: Update-Rule Resolution Event |
